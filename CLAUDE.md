...
**Never hand-edit `plugins/` or the root `README.md`.** Everything under `plugins/` is generated by `assemble.sh` from the source repos, and `README.md` is copied from batterie-de-savoir's `marketplace/README.md` (plugin table rendered from `brigade.toml`) — it is the suite's only public face now that most source repos are private (bds-mokava, 2026-09-23). To change a plugin: change its source repo, then either wait for the daily workflow or run `./assemble.sh` locally and commit. A hand-edit here is drift waiting to be clobbered.
