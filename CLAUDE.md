...
Not to be confused with **batterie-de-savoir** — the two are a **source/artifact pair**: that one is where the suite is *authored* (the docs umbrella — brigade.toml, Jekyll site — and the source of the suite-level `batterie` plugin); this one is what clients consume. Don't retire bds in a tidy-up: assessed 2026-06-11 — it sources the suite plugin, holds the brigade registry, and authors this repo's README. (It also used to serve a public Pages docs site; since it went private that site 404s — the README here replaced it as the public face, 2026-09-23.)

## The iron rule

**Never hand-edit `plugins/`, `.claude-plugin/marketplace.json` or the root `README.md`.** Everything under `plugins/` is generated by `assemble.sh` from the source repos; `marketplace.json` is generated from the vendored plugin.json files plus the `CATEGORIES` map beside `PLUGINS` (bds-dekava — to rename or delist a plugin, edit `PLUGINS` and `CATEGORIES` and nothing else; the run prunes the old `plugins/` dir); and `README.md` is copied from batterie-de-savoir's `marketplace/README.md` (plugin table rendered from `brigade.toml`) — it is the suite's only public face now that most source repos are private (bds-mokava, 2026-09-23). To change a plugin: change its source repo, then either wait for the daily workflow or run `./assemble.sh` locally and commit. A hand-edit here is drift waiting to be clobbered.
...
- **Version ratchet** — a plugin whose vendored content changed without the **suite** version bumping is **quarantined, not fatal** (2026-06-17, `bds-pujaki`; suite-level since the single-version cutover — it compares content against the last-published *suite* version, because every vendored plugin.json is stamped to that one number): the laggard's vendored dir is reverted to its last-published version (the marketplace stays consistent — last-good, not stale-new-content) and the *healthy* plugins still publish that run; a final workflow step then fails the run **red**, naming the laggard, so a missed bump can't hide behind green. One repo's missed bump no longer wedges the whole bus. **Since 2026-09-09 the quarantine self-heals** (`bds-hajeli`): instead of going red, the final step bumps the suite patch version in batterie-de-savoir with `publish.py` (an auto-generated CHANGELOG line naming each laggard and its newest commit subject), pushes it over the `BDS_DEPLOY_KEY` write deploy key, and dispatches a follow-on run, which publishes the laggards at the new version. Red is now reserved for a real fault — the deploy key missing, or the bump itself failing — because the run was red every day from 2 to 8 September and nobody acted on seven emails. `/batterie:publish` remains the "ship it now, with a written changelog line" verb; the daily auto-bump is the backstop. This is deliberately *softer* than the manifest/husk guards above — those are content **corruption** and stay hard-fail (never publish a husk); a version lag is recoverable. Without the ratchet at all, clients would see "no update available" over silently drifted content. Local escape hatch: `ASSEMBLE_NO_RATCHET=1`.
- **Shipped wheels** (2026-09-23, `bds-timule`) — `assemble.sh`'s `WHEELS` map builds each plugin CLI (bon, accomplis, passe, sonner), deglacer (into trousse) and jeton (into mise) into `plugins/<name>/wheels/`, so every consumer installs from this public repo and the workshop repos can be private. Builds go through a temp dir because `uv build` drops a `*` `.gitignore` into its output dir that would hide the wheel from git. Hatchling wheels are reproducible, so an unchanged source is not drift; a changed one is, and rides the suite bump like any content. Vendored mise's `[tool.uv.sources]` jeton line is rewritten to the shipped wheel and relocked; the build fails if its lock still names a spm1001 git repo. A private wheel source needs a `DEPLOY_KEY_<REPO>` line in the workflow's clone step, like any private source.
- **Deadman alerting — retired 2026-09-11.** The two Healthchecks checks (`bds-bidasi`) were folded into tube's daily estate-sweep, which reads this workflow's runs directly (red on main with age; a scheduled workflow silent for >3 days). The ping steps and their secrets are gone; `git log -S bds-bidasi` has the history.
