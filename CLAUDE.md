...
- **Private `batterie-home`** — a local `dist/batterie-home/` tree (gitignored): **mise only**, transformed to **mise-home** by `transforms/make-mise-flavour.sh` (identity strings rewritten, planetmodha OAuth client swapped in, a guard failing loud on any leftover ITV-identifying string). It derives from the **just-vendored public mise**, so both marketplaces carry identical runtime bytes and the same suite version — drift is structurally impossible. Emitted only when `MISE_HOME_CRED` points at the planetmodha `credentials.json`. **mise-home only, deliberately** (`bds-rikeno`, 2026-07-13): a whole-suite mirror was built then reversed — each CLI-backed plugin's `ensure-*.sh` SessionStart hook auto-installs its CLI + injects its rules shard, so mirroring drags per-session noise onto a family machine for tools nobody asked for. batterie-home's only reason to exist is the planetmodha cred; everything else is public. Instead, `assemble.sh` appends a **family stub** to mise-home's instruction shard (auto-loads via `~/.claude/rules/`) telling a family Claude the rest is public + how to add it — don't remove it, and don't re-mirror the suite. **In CI this is fully wired** (`bds-susugu`): the workflow materialises the cred from the `MISE_HOME_CRED_JSON` secret and pushes the output to [`spm1001/batterie-home`](https://github.com/spm1001/batterie-home) over a write deploy key (`BATTERIE_HOME_DEPLOY_KEY` secret) — one run, both marketplaces. Secrets absent → private half skips gracefully, public unaffected — but the skip can't stay silent: the `batterie-home-publish` deadman (below) starves and emails within ~31h (`bds-bidasi`).
