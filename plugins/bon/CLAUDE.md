...
**This repo went PRIVATE in the 2026-09-23 security triage; it was public before that**, so everything pushed before then (handoffs, `understanding.md`, `docs/`) was world-readable and may still be cached elsewhere. It also means plugin users can't see this repo unless they are collaborators. Asha (`asha-modha`) and Isaac (`zacmod`) were added with write access to every private suite repo on 2026-09-23, so family fixes can land as PRs again. Anyone else's report reaches us by message: read it as a pull request with nowhere to land. The public-era guidance below still applies if the repo is ever made public again. (Handoffs moved out of `.bon/` to the visible root `handoffs/` in the 2026-08-22 migration, and `.bon/handoffs/` stopped being a resolution rung at all in bon-sedoze; `.bon/` is still tracked here for the bottle and understanding.md. The board items themselves stay private only by accident of backend: bon's own board is Dolt-backed, so there's no items.jsonl here.) Most sibling repos — carnet, mary-bujournal, mary-bujournal-engine — are private, and that asymmetry is the trap. A finding about a live service belongs in the repo that owns the service, with only a pointer left here; a handoff describing an unfixed weakness publishes that weakness. One command settles which world you're in: `gh repo view <repo> --json visibility`.
