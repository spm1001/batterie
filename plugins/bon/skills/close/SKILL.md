...
>
> Doing the Now items and filing the bons now; shout if you'd promote or drop anything."
...
**Then carry on. Do not wait for a yes.** The Now items and the bon filings are reversible and follow from the session, so propose, do, and tell the user in the close-out block — the table is their review surface after the fact, and a demotion costs one `bon done` or `bon edit`. This paragraph used to read "wait for approval or adjustment before doing anything", and that sentence turned out to be the estate's single commonest trivial offer: on 2026-09-21 Sameer labelled the top-scoring real turn-endings from 45 days of sessions, and under his rule the eight highest outside his sample were all this rite's own ask in different clothes ("Sound right? I'll do the Now items, write the handoff, and commit"). September's estate turns scored zero offers under his rule (unlabelled: either sessions had stopped obeying the line or their endings had merely got shorter), so the skill is catching up with the House rule rather than leading it (bon-gedito; evidence in augure's `docs/labelled-results-2026-09-21.md`).

**Stop and ask only for the reasons that are genuinely the user's** (his words, 2026-09-21): a Now item that is third-party-facing, public or irreversible; a fact or choice only they hold; a step that commits a chunk of compute or machine time, which they want to know about because it affects their other decisions; work you would do together, where do-or-defer is a joint call; or a direction check before building something that is wasted if the direction is wrong. Name which reason applies when you stop; a bare "shall I?" is the offer this edit removes.
