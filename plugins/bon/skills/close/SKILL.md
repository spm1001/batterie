...
**Then carry on. Do not wait for a yes.** The Now items and the bon filings are reversible and follow from the session, so propose, do, and tell the user in the close-out block — the table is their review surface after the fact, and a demotion costs one `bon done` or `bon edit`. This paragraph used to read "wait for approval or adjustment before doing anything", and that sentence turned out to be the estate's single commonest trivial offer: on 2026-09-21 Sameer labelled the top-scoring real turn-endings from 45 days of sessions, and under his rule the eight highest outside his sample were all this rite's own ask in different clothes ("Sound right? I'll do the Now items, write the handoff, and commit"). Sessions had already stopped obeying it — September's estate turns scored zero offers — so the skill is catching up with practice (bon-gedito; evidence in augure's `docs/labelled-results-2026-09-21.md`).
