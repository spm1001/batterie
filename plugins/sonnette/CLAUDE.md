...
**Mesh-at-birth for any session (aby-pafada, 2026-07-19):** `claudem` (plain Teams) or `-m` on `claudev`/`claudeov`/`claudefv` appends `--dangerously-load-development-channels plugin:sonnette@batterie` — one trust dialog per launch, then inbound surfaces as `<channel>` tags. **Teams-billed only** — Vertex sessions can't bind channels ("Channels are not available on third-party providers"); the plugin's outbound tools still work there. Dialog-free-everywhere (allowlist) is **externally blocked**: on Teams-with-server-switch the `/etc/claude-code/managed-settings.json` `allowedChannelPlugins` is server-shadowed (measured dead 2026-07-19, aby-lesefu) — waits on Anthropic (#58152 / a Teams console allowlist field).

**Landmine — two servers in aboyeur's OWN cwd:** this repo is the only place with a project `.mcp.json` conductor server, so with sonnette also installed a session here loads BOTH (project `bun src/…` + plugin), deriving the same agentId. They no longer flap (aby-suwawo fixed in 1.16.1: a shared `owner.pid` makes the younger yield permanently to the live older sibling — age, not liveness, so the aby-tarafo restart-survivor still revives), but two servers still run with one yielded. If mesh acts oddly *in aboyeur*, that's why: `claude plugin disable sonnette` locally (keep the `.mcp.json` dev hot-path), or work from another cwd. Regression repro: `tests/suwawo-two-process.mjs`.
...
Pre-alpha. Daemon plumbing built and tested. Mesh connectivity validated via Channels MCP (conductor-channel.ts). Peer review loop proven with live mesh round-trips. Interactive mode supersession fix in place (aby-tarafo); same-id double-server war fixed (aby-suwawo, 1.16.1). **Mesh is shipped for real use:** sonnette in the batterie marketplace (aby-zufefu), mesh-at-birth via `claudem`/`-m` (aby-pafada), one-shot phone-a-friend via `/consult`. The full three-rung capability ladder (one-shot free / mesh-at-birth via flag / dialog-free blocked on Anthropic) is mapped in `.bon/understanding.md`. Next: aby-lezuhu (Bun docs sweep), aby-darode + the statusline capability indicator, aby-luviwu (graduate /consult to trousse), aby-rinisa (design review, largely answered).
