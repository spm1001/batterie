...
**`ai-title` is absent from phone-spawned sessions, which makes them anonymous locally.** A session created from the Claude mobile app records `entrypoint=sdk-cli` and never gets an `ai-title`; the title you see in the app lives server-side only. So it shows up nameless in `claude --resume` and you must pass its id explicitly. An interactive session (`entrypoint=cli`) usually does get one — but not always, so absence of a title is not proof of a phone origin. This cost a real session its work: three duplicate phone sessions were archived to save compute, and identifying which had progressed furthest took eight probes because none had a name. Measured era-matched on one estate (sessions since 2026-08-15, so every one of them post-dates `ai-title` itself): **v4 UUIDs 21/25 carry an `ai-title`; v5 UUIDs 0/12.** An earlier pass at this measurement sampled sessions at random and got a meaningless 0/30 against 1/30, because most of them pre-dated the field entirely: era-match the control.

**Corrected 2026-09-09 (CC 2.1.266): namelessness is real, but it is not why they are missing — the picker FILTERS SDK-spawned sessions out by design.** The lister in the bundle returns null with the debug line `filtered from /resume: entrypoint=…` whenever the transcript-head `entrypoint` is `sdk-cli`, `sdk-ts` or `sdk-py` and the current process is not itself an SDK entrypoint; the same pass drops `sessionKind` daemon/daemon-worker, `teamName` mismatches, sessions continued elsewhere (`continuedInSessionId`), and `bookkeepingOnly` transcripts. Measured live under hublot: the picker in a directory with 65 rows showed none for a 27-minute-old phone session, and a search for its opening words returned *No sessions match* while a titled control filtered to one row. The earlier "nameless, so nothing to render" reading in this paragraph was wrong: a nameless session that survives the filter is listed under its first prompt (the display falls back `agentName → customTitle → aiTitle → summary → firstPrompt → id`), though the picker's search matches `customTitle ?? aiTitle` only, so it stays unsearchable. Consequence: no title backfill can surface a phone session in `/resume`; pass the uuid explicitly (`teleport-id.sh` prints the line), or use a lister of your own (`remote-sessions.sh`, Guéridon's session index). **While the session is live, its registry record (`~/.claude/sessions/<pid>.json`, or the commis seat's dir) already pairs `bridgeSessionId` with `sessionId`** — `teleport-id.sh` reads that first and warns that a live pid holds the transcript, because a plain `--resume` beside a live driver puts two writers on one JSONL. A phone session that has hit its model limit is still a live bridge child until something ends it (SIGTERM by pid is graceful and resumable).
